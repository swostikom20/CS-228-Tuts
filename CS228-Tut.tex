...
Given $n$, consider the set of all words in $\{0, 1\}$ with length $n - 1$. Consider any DFA $A = (Q, \{0, 1\}, \delta, q_0, F)$ accepting $L_n$. Suppose there are distinct words $v$ and $w$ that end in the same state $q \in Q$.
...
$vx$ has $n$th bit from the right = 0 $\quad \Longrightarrow \quad$ $vx \notin L_n$. Thus, $q' \notin F$. \\
$wx$ has $n$th bit from the right = 1 $\quad \Longrightarrow \quad$ $vx \in L_n$. Thus, $q' \in F$. \\
Contradiction!\\
